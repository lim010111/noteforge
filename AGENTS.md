...
- **CRITICAL**: frontmatter는 **allowlist** 외 필드를 공개 HTML/meta에 노출하지 말 것. allowlist: `title`, `description`, `date`, `updated`, `tags`, `aliases`, `cover`, `thumbnail`, `author`, `draft`, `public`, `slug`, `permalink`, `lang`, `featured`.
  - `cover`: 포스트 hero 배경 이미지. `/attachments/<rel>` 값은 public attachment closure에 있을 때만 노출.
  - `thumbnail`: 리스트용 썸네일 이미지. 미설정 시 privacy-gated hero 이미지로 폴백.
- **CRITICAL**: 게시물 미리보기 서문은 allowlisted `description`을 우선 사용하고, 없을 때만 이미 privacy 파이프라인을 통과한 `rendered.html` 앞부분에서 excerpt를 만든다. raw vault body나 allowlist 밖 frontmatter를 새로 읽지 말 것.
...
## Agent 운용
이 프로젝트는 AI agent (Claude Code · Codex CLI) 가 작업하는 것을 가정한다. agent log / eval placeholder는 [evals/README.md](./evals/README.md) 참조. 모든 디렉터리의 `CLAUDE.md` 는 동일 내용 `AGENTS.md` 와 짝을 이뤄야 하며 검증은 `pnpm validate:context-paths` 가 수행한다.
