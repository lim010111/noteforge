---
description: Obsidian-Publish-OSS 하네스 — phase 상태 자동 감지 후 다음 행동 수행
argument-hint: "[new <name>|resume|status|run]"
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, Agent, AskUserQuestion
---

이 프로젝트는 Harness 프레임워크를 사용한다. `/harness` 슬래시 커맨드로 호출되며, **아래 절차를 반드시 순서대로 수행**한다.
...
호출 즉시 다음을 **병렬로** 읽어 내부 상태를 구성한다. Glob + 다중 Read + 필요 시 Bash 병렬.
...
- `README.md`
- `TODO.md`
- CLAUDE.md에 `정식 계획은 …plans/…` 형식으로 참조된 plan 파일 (경로 존재 시)
...
필수 Bash 체크:
- `git rev-parse --is-inside-work-tree` (git repo 여부)
- git repo면 `git status --porcelain`, `git branch --show-current`

**상태 불일치 감지** (다음 중 하나라도 참이면 S9로 분기):
...
초기 로드 결과로 아래 조건을 위에서 아래로 평가해 **첫 매치 상태**를 선택하고 해당 행동을 수행한다.

| # | 감지 조건 | 상태 | 행동 |
|---|---|---|---|
| S0 | `git rev-parse --is-inside-work-tree` 실패 | **git 미초기화** | AskUserQuestion으로 초기화 승인 확인 → `git init` → `git add -A` → `git commit -m "chore: initial commit"` → 상태 재감지 (대개 S1로 이어짐) |
| S9 | 초기 로드에서 상태 불일치 감지 | **복구** | 구체 불일치 사유를 보고 → AskUserQuestion으로 복구 옵션 제시 (수동 편집 안내 / `index.json` 재생성 / 해당 phase 재설계). **사용자 확인 없이 자동 수정하지 않는다** |
| S1 | `phases/` 없음 또는 `phases/index.json` 없음 | **신규** | TODO.md·plan 파일 기반 첫 phase 후보 제안 → AskUserQuestion으로 scope 확정 → §4 설계 원칙 따라 step 초안 → §5 템플릿으로 파일 생성 → §6 실행 안내 |
...
| S3 | 모든 step 파일 작성 완료, 실행 시작 전 (전부 `pending` + `started_at` 없음) | **실행 대기** | phase 개요·step 목록 요약 + 실행 명령(`python3 scripts/execute.py <dir>`)만 제시. 커맨드 내부에서 실행하지 않는다 |
| S4 | 일부 step `completed` + 다음 `pending` | **진행 중** | 완료 step의 `summary`를 요약 + 다음 pending step 미리보기 + 재개 명령 안내 |
| S5 | 첫 non-completed step이 `error` | **에러** | `step{N}.md`, `step{N}-output.json`의 stdout/stderr/exitCode 읽고 원인 진단 → AskUserQuestion으로 복구 옵션 제시 (재시도 / step 지시 보강 / 수동 처리) → 선택 적용 후 재개 명령 안내 |
...
## § 3. 인자(`$ARGUMENTS`) 기반 분기 강제

| 인자 | 동작 |
|---|---|
| (빈 문자열) | §2 상태 머신 자동 분기 |
| `new <name>` | S1/S2 흐름 강제. `<name>` 누락 시 AskUserQuestion으로 받음. `phases/<name>/` 이미 존재 시 중단 알림 |
| `resume` | 활성 phase의 error/blocked step으로 진입 (S5/S6). 없으면 `status`처럼 요약만 보고 "복구할 상태 없음" 알림 |
| `status` | §1 컨텍스트 로드 + §2 상태 요약**만**. Edit/Write/Bash 쓰기 명령 금지. AskUserQuestion도 하지 않는다 |
| `run` | S3/S4 요약 + 실행 명령 제시(실행은 사용자 몫) |
| 기타 | 위 인자 목록을 안내하고 중단 |

**`<name>` 유효성**: kebab-case slug(`^[a-z0-9-]+$`). 위반 시 AskUserQuestion으로 재입력.
...
2. **자기완결성** — 각 step 파일은 독립된 Claude 세션에서 실행된다. "이전 대화에서 논의한 바와 같이" 같은 외부 참조 금지. 필요한 정보는 전부 파일에 적는다.
...
5. **AC는 실행 가능한 커맨드** — `pnpm -r typecheck && pnpm lint && pnpm test` 같은 실제 실행 가능한 검증 커맨드를 포함. (주의: 이 프로젝트에서 `lint`/`test` 스크립트는 **루트에만** 존재하므로 `-r` 없이 호출한다.)
...
- `summary`/`error_message`/`blocked_reason`은 step 수행 중 child Claude 세션이 기록.
...
3. 결과에 따라 `phases/<dir>/index.json`의 해당 step을 업데이트:
...
- 가드레일 주입 — CLAUDE.md + docs/*.md 내용을 매 step 프롬프트에 포함 (∴ step.md에서 CLAUDE.md/docs 중복 기재 불필요)
...
에러/차단 복구는 `/harness resume`으로 진입한다 (§2 S5/S6 처리).

---

## § 7. 금지/주의

- 커맨드 내부에서 execute.py를 자동 실행하지 않는다 (사용자가 터미널에서 직접 실행).
...
- `phases/**`를 사용자가 수동 편집한 흔적이 있으면(`git diff`) 그 편집을 덮어쓰지 않는다.
- Uncommitted 변경이 있는 상태에서 S1/S2 파일 생성 시, 먼저 사용자에게 stash/commit할지 묻는다.
- 최초 분석 결과를 장황하게 나열하지 말고, **지금 이 상태에서 사용자가 해야 할 한두 가지**를 명확히 제시한다.
