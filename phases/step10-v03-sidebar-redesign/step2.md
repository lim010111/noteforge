...
v0.2 토큰을 그대로 두고 *추가만* 한다. 다음을 라이트(`:root`) + 다크(`[data-theme="dark"]`/`@media (prefers-color-scheme: dark)`) 양쪽에 추가:

1. **보조 accent 1개** — `--color-accent-secondary`, `--color-accent-secondary-hover`. warm 계열만(보라/인디고/네온 금지). 본문 대비 ≥ 4.5:1 (라이트/다크 둘 다).
2. **카테고리 accent 슬롯** — `--color-accent-cat-1` … `-N` (N은 design/TOKENS.md 결정 — 4~6). 의미 중립. 라이트/다크 양쪽.
3. **새 surface tier 1개** — `--color-bg-sidebar` (또는 design/TOKENS.md 결정명). `--color-bg-page`와 `--color-bg-surface` 사이의 단차.
...
- `tokens.css` 파일을 읽어 다음 변수가 `:root`에 모두 정의되어 있는지 단순 grep:
  - `--color-accent-secondary`
  - `--color-accent-secondary-hover`
  - `--color-accent-cat-1` (최소 1개 슬롯 — N은 디자인 SSOT)
  - 새 surface tier 변수 1개
- 같은 변수들이 `[data-theme="dark"]` 블록에도 정의되어 있는지 검증.
...
# 신규 토큰 존재 sanity
grep -c '\-\-color\-accent\-secondary' packages/theme-default/src/styles/tokens.css   # ≥ 2 (root + dark)
grep -c '\-\-color\-accent\-cat\-1'     packages/theme-default/src/styles/tokens.css   # ≥ 2

# v0.2 토큰 보존 (회귀 가드)
grep -c '\-\-color\-text\-link'         packages/theme-default/src/styles/tokens.css   # 변화 없음
```
