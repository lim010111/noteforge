...
폴더 트리 빌드는 `packages/core` 재사용 가치가 낮다고 판단해 `apps/blog/src/lib/folderAggregation.ts`에 둔다. 입력이 Astro `CollectionEntry`(Content Layer 타입)이고 출력이 `Sidebar`/`FolderIndex` props 형태이므로, 코어 재사용 시점에서는 어차피 어댑터 레이어가 다시 필요하다. 핵심 트레이드오프는 *코어 의존성 제거 vs 어댑터 1개 추가*였고, 어댑터 추가가 더 가벼웠다 — 핵심 책임(privacy 판정·렌더 파이프라인)을 코어에 응집시키고, 어댑터 책임(Astro 콜렉션 → 트리 형태 매핑)은 콘텐츠 레이어가 사는 곳 옆에 두는 편이 코어를 vault-agnostic으로 유지하는 데 유리하다. 슬롯 매핑 해시(`categorySlot.ts`)만 코어로 — 그건 vault-agnostic.
