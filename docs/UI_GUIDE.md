...
이 문서는 fork 사용자를 위한 외부 SSOT다. 토큰의 권위 있는 출처는 [`packages/theme-default/src/styles/tokens.css`](../packages/theme-default/src/styles/tokens.css)이며, 본 가이드는 그 CSS 파일을 사람이 읽을 수 있는 형태로 거울처럼 옮긴 것이다. 두 문서가 충돌할 경우 코드(CSS 파일)가 SSOT다. v0.2 본문은 [`docs/UI_GUIDE.v0.2.md`](./UI_GUIDE.v0.2.md)에 보존되어 있다(v0.1은 [`docs/UI_GUIDE.v0.1.md`](./UI_GUIDE.v0.1.md)).
...
| **Hairline rules instead of shadows** (1px `--color-border` 어디서나 + 옵션 `--shadow-1` 1개만) | 섹션 디바이더, 카드, embed aside, 테이블 행, 스크롤된 sticky 헤더(v0.6에서 wire-up — [ADR-0014](./adr/0014-sticky-header-shadow-on-scroll.md)) | 카드/aside/칩의 elevation 힌트로 그림자 사용 금지 — 보더로 표현. 토큰 tier에 멀티 스텝 그림자 스케일이 존재하지 않음. | **변경 없음.** 사이드바-본문 분리는 단일 우측 보더. rail row는 `divide-y` 헤어라인. v0.3는 새 그림자 tier 도입하지 않는다. v0.6는 sticky 헤더 surface만 1px sentinel + IntersectionObserver로 wire-up — 같은 `--shadow-1` 재사용. |
...
토큰은 [`packages/theme-default/src/styles/tokens.css`](../packages/theme-default/src/styles/tokens.css)의 CSS 변수로 정의된다. 본문 텍스트와 페이지/사이드바 배경 대비는 라이트·다크 양쪽 WCAG 상대휘도 계산으로 ≥ 4.5:1 검증됨. 본 절의 모든 토큰명·hex 값은 그 CSS 파일과 1:1로 일치하며, 충돌 시 코드가 SSOT다.
...
각 컴포넌트의 시각·모바일 동작 SSOT는 본 가이드 그 자체이며, 구현 정합은 [`packages/theme-default/src/components/`](../packages/theme-default/src/components/)와 [`packages/theme-default/src/styles/components.css`](../packages/theme-default/src/styles/components.css)에서 검증한다. 본 절의 모든 anatomy/모바일 규약은 위 코드와 1:1, 충돌 시 코드가 SSOT다.
...
## 9. 모션 *(v0.6 갱신 — 테마/메뉴 토글 cross-fade + 스크롤 헤더 shadow)*

| 토큰 | 값 | 용도 |
|---|---|---|
| `--duration-fast` | 100ms | 링크 underline 두께 |
| `--duration-base` | 150ms | 링크 색, 버튼 bg, **테마 토글 아이콘 cross-fade + rotate**, **모바일 메뉴 hamburger↔X cross-fade + rotate**, **폴더 chevron 회전**, **스크롤된 sticky 헤더 `--shadow-1` 페이드** |
| `--duration-menu` | 200ms | 모바일 메뉴 슬라이드, **사이드바 드로어 슬라이드** |
| `--ease-out` | `cubic-bezier(0.2, 0, 0, 1)` | 감속, 기본 ease |
| `--ease-linear` | `linear` | 색만 트랜지션 |

- 허용: 위 표의 트랜지션, 사이드바 드로어 슬라이드, 폴더 chevron 회전, 헤딩 앵커 호버 페이드, 테마 토글 아이콘 cross-fade(+12°↔0°), 모바일 메뉴 hamburger↔X cross-fade(±90°), 스크롤된 sticky 헤더 `--shadow-1` 페이드(센티넬 + IntersectionObserver — [ADR-0014](./adr/0014-sticky-header-shadow-on-scroll.md)).
- 금지: 페이지 전환 fade-in/slide-up, scroll-reveal, 패럴랙스, 카드 lift on hover, 자동 캐러셀, **폴더 펼침의 height-animate**(layout shift 비용이 무거우며 v0.3 정책에 어긋남), 액센트 사이 gradient, 펄스/글로우.
- 토글 아이콘은 *cross-fade + 미세 rotate* — display swap이 아니다. 두 SVG가 같은 박스 안에 겹쳐 있고 `opacity` + `transform`만 트랜지션한다(layout shift 0). 360° 회전, 스피너, "스카이" 트릭은 §6 "Restraint over decoration"과 충돌.
- `--duration-slow` / spring 토큰은 의도적으로 미제공.
- **`prefers-reduced-motion: reduce` 의무 지원**: `base.css`에서 위 모든 duration을 0으로 덮어쓴다(`animation-duration / transition-duration: 0.001ms !important`). 새 컴포넌트도 동일하게 준수.
