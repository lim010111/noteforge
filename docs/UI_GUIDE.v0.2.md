> **보존 문서.** 이 파일은 v0.2 시점의 UI 가이드 스냅샷이며, v0.3부터는 [UI_GUIDE.md](./UI_GUIDE.md)가 정본이다. v0.2의 계약(editorial-technical 골격, 라이트/다크 토큰 시스템, serif/sans/mono triad self-host, 7개 안티패턴, privacy 시각 계약 14절)은 v0.3에서도 그대로 유지되므로 참조용으로 남긴다.
...
이 문서는 v0.2 시점의 보존본이다. 활성 가이드는 [`docs/UI_GUIDE.md`](./UI_GUIDE.md)이며, 토큰의 SSOT는 [`packages/theme-default/src/styles/tokens.css`](../packages/theme-default/src/styles/tokens.css)다. 본 v0.2 보존본의 수치는 v0.2 출시 시점의 토큰 값이며, 이후 변경분은 활성 가이드와 CSS 파일을 참조하라.
...
토큰은 [`packages/theme-default/src/styles/tokens.css`](../packages/theme-default/src/styles/tokens.css)의 CSS 변수로 정의된다. 본문 텍스트와 페이지 배경 대비는 라이트·다크 양쪽 WCAG 상대휘도 계산으로 ≥ 4.5:1 검증됨. 본 절의 모든 토큰명·hex 값은 v0.2 출시 시점 CSS 토큰의 스냅샷이다.
...
각 컴포넌트의 시각·모바일 동작은 v0.2 출시 시점 컴포넌트 anatomy의 스냅샷이다. 활성 컴포넌트 스펙은 [`docs/UI_GUIDE.md`](./UI_GUIDE.md) §7과 [`packages/theme-default/src/components/`](../packages/theme-default/src/components/)에서 확인할 것.
