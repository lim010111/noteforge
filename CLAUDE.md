...
정식 계획은 `/home/shine/.claude/plans/public-fizzy-patterson.md`.
...
│   ├── astro-integration/  # @obpub/astro  — Astro Content Layer + watcher 어댑터
│   ├── theme-default/      # @obpub/theme-default — 레퍼런스 테마
│   └── cli/                # @obpub/cli    — obpub dev/build/audit/status
└── apps/
    └── blog/               # 도그푸드 사이트
```
...
- 머지 전 로컬: `pnpm -r typecheck && pnpm -r lint && pnpm -r test && pnpm --filter apps/blog build`.
